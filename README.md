...
actions are javascript objects carried in type attributes to carry payload info from app to store, -reducers are functions that dtermine change in an apps state. -store holds the state tree in our app 

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is the "global" state that all components in the component tree can access. -Component local state is state that is local to a single component and cannot be seen outside of this component
